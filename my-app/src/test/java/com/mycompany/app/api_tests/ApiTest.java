...
class ApiTest {
	...
	void testAll() {
		...
		String commithash = System.getenv().getOrDefault("COMMIT_HASH", "dummy COMMIT_HASH");
		Results results = Runner.path("classpath:com/mycompany/app/api_tests")
			.systemProperty("url.base", apiHostServer)
			.systemProperty("AUTH_TOKEN", authtoken)
			.systemProperty("COMMIT_HASH", commithash)
			.parallel(1);
